package ad.expression
{
	...
	public class Lexer
	{
		private function Lexer(tokensCapacity:int, allTokensCapacity:int)
		{
			this.input = "";
			this.locale = locale;
		...
		*/
		...
		{
			if (token.getType() != TokenType.Whitespace && token.getType() != TokenType.Special)
				tokens.add(token);
			allTokens.add(token);
		}
		...
		*/
		private boolean ws()
		{
			...
			{
				int length = lengthRef.get();
				add(new Token(input.substring(current, current + length), TokenType.Whitespace, current));
				current += length;
				return true;
			}
		...
		*/
		private boolean ob()
		{
			if (match('('))
			{
				add(new Token(input.substring(current, current + 1), TokenType.ArgumentBeginOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean cb()
		{
			if (match(')'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.ArgumentEndOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean ocb()
		{
			if (match('{'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.DataBeginOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean ccb()
		{
			if (match('}'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.DataEndOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean vline()
		{
			if (match('|'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.Pipe, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opExponent()
		{
			if (match('^'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.ExponentOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opPlus()
		{
			if (match('+'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.AdditionOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opMinus()
		{
			if (match('-'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.SubtractionOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opMultiply()
		{
			if (match('*'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.MultiplicationOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opDivide()
		{
			if (match('/'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.DivisionOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opPercent()
		{
			if (match('%'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.ModuloOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opLess()
		{
			if (match('<'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.StrictLessOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opGreater()
		{
			if (match('>'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.StrictGreaterOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opEqual()
		{
			if (match('='))
			{
				add(new Token(input.substring(current, current + 1), TokenType.EqualityOperator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean opNotEqual()
		{
			if (match("<>"))
			{
				add(new Token(input.substring(current, current + 2), TokenType.InequalityOperator, current));
				current += 2;
				return true;
			}
		...
		*/
		private boolean opLessOrEqual()
		{
			if (match("<="))
			{
				add(new Token(input.substring(current, current + 2), TokenType.LessOperator, current));
				current += 2;
				return true;
			}
		...
		*/
		private boolean opGreaterOrEqual()
		{
			if (match(">="))
			{
				add(new Token(input.substring(current, current + 2), TokenType.GreaterOperator, current));
				current += 2;
				return true;
			}
		...
		*/
		private boolean comma()
		{
			if (match(','))
			{
				add(new Token(input.substring(current, current + 1), TokenType.Delimiter, current));
				current++;
				return true;
			}
		...
		*/
		private boolean semicolon()
		{
			if (match(';'))
			{
				add(new Token(input.substring(current, current + 1), TokenType.Terminator, current));
				current++;
				return true;
			}
		...
		*/
		private boolean intNumber()
		{
			...
			{
				int length = lengthRef.get();
				add(new Token(input.substring(current, current + length), TokenType.IntegralNumber, current));
				current += length;
				return true;
			}
		...
		*/
		private boolean floatNumber()
		{
			...
			{
				int length = lengthRef.get();
				add(new Token(input.substring(current, current + length), TokenType.FloatingPointNumber, current));
				current += length;
				return true;
			}
